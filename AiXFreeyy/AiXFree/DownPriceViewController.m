...
#define XFreeurl @"http://iappfree.candou.com:8080/free/applications/sales?currency=rmb&page=%ld"
@interface DownPriceViewController ()
...
-(void)loadData
{
    ...
    [man GET:[NSString stringWithFormat:XFreeurl,self.starIndex+1] parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        for (NSDictionary *app in movieDict[@"applications"]) {
            DownPriceModel *appmodel = [[DownPriceModel alloc] initWithDic:app];
...
{
    DownPriceTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"DownpriceCell"];
    if(self.isSearching)  //是否进行搜索的开关控制
    {
        DownPriceModel*Model = self.resultArray[indexPath.row];
        [cell reflushUI:Model];
    }else{
        DownPriceModel*model = self.data[indexPath.row];
        [cell reflushUI:model];
    }
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self.tableView registerNib:[UINib nibWithNibName:@"DownPriceTableViewCell" bundle:nil] forCellReuseIdentifier:@"DownpriceCell"];
    [self loadData];
...
-(void)getSearchData
{
    for (DownPriceModel*model in self.data) {
